...
namespace DocumentLibrary.Viewer
{
    public static class CssValue
    {
        ...
        private static object DetectSource(string path)
        {
            ...
            else if (path.StartsWith("data:"))
            {
                try
                {
                    int d = int.Parse(path.Remove(0, 5));
                    MemoryStream m = new MemoryStream(DocumentLibrary.ArsslenDocument.CurrentDocumentRessources[d]);
                    File.WriteAllBytes("C:\\a.jpg", DocumentLibrary.ArsslenDocument.CurrentDocumentRessources[d]);
        ...
        public static Image GetImage(string path)
        {
            ...
            try
            {
              ...
              else if (url != null)
                {
                    ...
                    string filename = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + @"\ASDOC\" + DateTime.Now.ToFileTimeUtc().ToString() + Path.GetExtension(url.OriginalString);

                    wb.DownloadFile(url.OriginalString, filename);
